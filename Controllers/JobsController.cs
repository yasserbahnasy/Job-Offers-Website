...
namespace Job_Offers_Website.Controllers
{
    ...
    public class JobsController : Controller
    {
        ...
        public ActionResult Create(Job job,HttpPostedFileBase UploadJob)
        {
            if (ModelState.IsValid)
            {
                string path = Path.Combine(Server.MapPath("~/uploads"),UploadJob.FileName);
                UploadJob.SaveAs(path);
                job.JobImage = UploadJob.FileName;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(Job job, HttpPostedFileBase UploadJob)
        {
            if (ModelState.IsValid)
            {
